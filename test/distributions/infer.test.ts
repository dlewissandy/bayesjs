...
function infer (
  ...
  evidence: { [name: string]: string[] },
  precision: number,
) {
  const dist = engine.getJointDistribution(
    Object.keys(event),
...
describe('should infer the correct liklihood for a distribution', () => {
  ...
  it('infer should throw when only invalid levels are provided for a parent variable', () =>
    expect(() => infer({ node1: ['T'] }, { node2: ['Z'] }, 4)).toThrow())
  it('it should infer the correct probability for a junction forest.', () => {
    const disconnected = new InferenceEngine({
      ...
      B: {
        levels: ['T', 'F'],
        parents: ['A'],
        potentialFunction: [0.2, 0.3, 0.3, 0.2],
      },
      C: { levels: ['T', 'F'], parents: [], potentialFunction: [0.2, 0.8] },
      D: {
        levels: ['T', 'F'],
        parents: ['C'],
        potentialFunction: [0.35, 0.15, 0.15, 0.35],
      },
      E: { levels: ['T', 'F'], parents: [], potentialFunction: [0.6, 0.4] },
      F: {
        levels: ['T', 'F'],
        parents: ['E'],
        potentialFunction: [0.45, 0.05, 0.05, 0.45],
      },
    })
    ...
    expect(observed.map(x => Number.parseFloat(x.toPrecision(4)))).toEqual(
      expected,
    )
